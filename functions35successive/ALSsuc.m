...
% val1 =  evalAuTTsuc(u,psiDatas,v,p,tau);
% val2 =  evalAuTT(u,psiDatas{1},v,p,tau);
...
function psiDataNew = optimizeMode(mode,u,psiDatas,v,p,tau,N)
    ...
    %interpolationErr = abs(poly(0.5,0.5) - evalAB(0.5,0.5))
    %interpolationErr = abs(poly(2,2) - evalAB(2,2))
    %smallEig = eigs([2*alpha(1),alpha(3);alpha(3),2*alpha(2)],1,'smallestabs')
