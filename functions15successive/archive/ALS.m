...
decay = 1e54; type = 'int'; iters = 1; val(iters) =  evalAuTT(u,psiData,v,p,tau); psiData = renormalize(psiData);
while decay > 1e-3*abs(val(iters)) && iters < 150
    for mode = 1:N
        psiDataNew = optimizeMode(mode,u,psiData,v,p,tau,N,type);
        iters = iters+1;
...
function psiDataNew = optimizeMode(mode,u,psiData,v,p,tau,N,type)
%% symbolic approach
if type == 'sym'
    a = sym('a'); b = sym('b');
    if mode == 1
        psiDataSym = [a,b; psiData(2:end,:)];
    elseif mode == N
        psiDataSym = [psiData(1:end-1,:);a,b];
    else
        psiDataSym = [psiData(1:mode-1,:);a,b; psiData(mode+1:end,:)];
    end
    coeff = evalAuTT(u,psiDataSym,v,p,tau);
    [x,y] = coeffs(coeff);
    x = double(x); %can I even solve the lineary sytem below in a symbolic fashion?
    alpha1 = x((y == a^2));
    alpha2 = x((y == b^2));
    alpha3 = x((y == a*b));
    alpha4 = x((y == a));
    ...
    minIsUniqueAndExists = min(eigs([2*alpha1,alpha3;alpha3,2*alpha2]) > 0);
    ...
    newparams = [2*alpha1, alpha3; alpha3, 2*alpha2]\[-alpha4;-alpha5];
    if mode == 1
    ...
    %% use interpolation instead of the symbolic approach (faster to compute)
if type == 'int'
    if mode == 1
